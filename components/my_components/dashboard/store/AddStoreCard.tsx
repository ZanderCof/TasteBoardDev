...
import { Plus, Store, Lock } from "lucide-react";
...
export const AddStoreCard = ({ disabled = false }: AddStoreCardProps) => {
  const [open, setOpen] = useState(false);

  if (disabled) {
    return (
      <div className="w-full relative overflow-hidden bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[2.8rem] p-10 flex flex-col items-center justify-center gap-6 h-full min-h-87.5">
        <div className="relative">
          <div className="w-20 h-20 rounded-[2rem] bg-slate-200 flex items-center justify-center text-slate-400 shadow-inner">
            <Lock size={32} strokeWidth={2} />
          </div>
          <div className="absolute -bottom-2 -right-2 p-2.5 bg-slate-200 rounded-2xl text-slate-400 shadow-xl border-4 border-white">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-400 font-medium max-w-50 leading-relaxed">
            Il piano attuale supporta <span className="font-bold">1 locale</span>. Aggiorna il piano per aggiungerne altri.
          </p>
        </div>
      </div>
    );
